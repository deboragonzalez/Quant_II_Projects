...
```{r}
ghostbusters <- c("Egon", "Peter", "Ray", "Winston")
```
...
```{r}
group <- rep(c("Control", "Treatment"), c(5,5))
```
...
```{r}
logicVec <- c(TRUE, TRUE, FALSE, TRUE, TRUE)
```
...
The vector printed all its elements with the exception of the 3rd element.We are telling R to look at vec1 one position at a time. If the corresponding value in logicVec is TRUE, we keep it. If it’s FALSE, we drop it.
